...
for %%i in (..\.mp4) do (%~dp0..\..\bin\ffmpeg -i "%%i" -c:v libx265 -vtag hvc1 -c:a copy "%%~dpni_temp.mp4" && del "%%i" && mv "%%~dpni_temp.mp4" "%%i" && %~dp0..\..\bin\ffmpeg -i "%%i" -c copy "%%~dpni_remux.mp4" && del "%%i" && mv "%%~dpni_remux.mp4" "%%i")
