...
for %%i in (%*) do (%~dp0..\..\bin\ffmpeg -i %%i -c:v copy -c:a copy "%%~dpni.mp4" && del "%%~dpni.mkv" && %~dp0..\..\bin\ffmpeg -i "%%~dpni.mp4" -c copy "%%~dpni_remux.mp4" && del "%%~dpni.mp4" && mv "%%~dpni_remux.mp4" "%%~dpni.mp4")